...
RESULTS_PHASE4_DIR = os.path.join(OUTPUT_DIR_ROOT, 'Results_Phase4')
CHECKPOINT_PHASE4_ROOT = os.path.join(OUTPUT_DIR_ROOT, 'checkpoints_Phase4')
os.makedirs(RESULTS_PHASE4_DIR, exist_ok=True)
os.makedirs(CHECKPOINT_PHASE4_ROOT, exist_ok=True)
...
def get_validation_augmentation(): return albu.Compose([])
...
# Discover best Phase 1 run from output.txt
...
phase1_pairs = [(os.path.join(UNLABELED_DIR, x), os.path.join(OUTPUT_DIR_ROOT, 'Predictions', 'Phase1', x)) for x in phase1_names]

all_unlabeled = os.listdir(UNLABELED_DIR)
phase2_unlabeled = [x for x in all_unlabeled if x not in phase1_names]

all_metrics = [metrics.IoU(threshold=0.5), metrics.Fscore(threshold=0.5), metrics.Accuracy(threshold=0.5)]
dice_loss = losses.DiceLoss()
focal_loss = losses.FocalLoss()
dce_loss = losses.DynamicCEAndSCELoss()

total_loss = base.HybridLoss(dice_loss, focal_loss, dce_loss, [1.0, 1.0, 1.0])
...
for run, seed in enumerate(seeds):
    print(f'\n=========================== Phase 2 run {run+1}/{n_runs} (seed {seed}) ============================')
    model_name = BASE_MODEL + '_padded_' + ENCODER + f'_run_{run}_selfSupervised_phase2'
    ...
    # Pick 50 unlabeled from Phase 2 remaining pool
    remaining_unsup_names_IDs = phase2_unlabeled.copy()
    random.shuffle(remaining_unsup_names_IDs)
    phase2_50_names = remaining_unsup_names_IDs[:50]

    phase2_pairs = [(os.path.join(UNLABELED_DIR, x), os.path.join(OUTPUT_DIR_ROOT, 'Predictions', 'Phase2', x)) for x in phase2_50_names]
    train_pairs = sup_train_pairs + phase1_pairs + phase2_pairs
    
    # Save the selected 50 for next phase
    with open(os.path.join(RESULTS_PHASE4_DIR, f'run_{run}_unsup_train.txt'), "w") as f:
        for name in phase2_50_names: f.write(name + "\n")

    checkpoint_loc = os.path.join(CHECKPOINT_PHASE4_ROOT, model_name)
    ...
    history_df.to_csv(os.path.join(RESULTS_PHASE4_DIR, f'run_{run}_training_metrics_history.csv'), index=False)

# Evaluate best run on test set
best_run_tuple = min(best_val_losses_across_runs, key=lambda x: x[1])
print(f"\nTraining for 5 Phase-2 runs completed. Best Run: {best_run_tuple[0]} with validation loss {best_run_tuple[1]}")
...
model.load_state_dict(torch.load(os.path.join(best_run_tuple[2], 'best_model.pth'))['state_dict'])
...
with open(os.path.join(BASE_DIR, 'output.txt'), 'a') as f:
    f.write(f"\n# Phase 4 & 5: Semi-Supervised Training Phase 2\n")
    f.write(f"- Tested 5 iterations of randomly selected 50 pseudo-labeled images (from Phase 2 pool) + 50 Phase 1 + 78 labeled images.\n")
    f.write(f"- Best Run: {best_run_tuple[0]} with validation loss: {best_run_tuple[1]:.4f}\n")
    f.write(f"- Evaluation of Phase 2 on Test Set:\n")
    f.write(f"     Accuracy: {test_logs['accuracy']:.4f}\n")
    f.write(f"     F1-score: {test_logs['fscore']:.4f}\n")
    f.write(f"     IoU: {test_logs['iou_score']:.4f}\n")
print("\nPhase 2 Results successfully appended to output.txt")
